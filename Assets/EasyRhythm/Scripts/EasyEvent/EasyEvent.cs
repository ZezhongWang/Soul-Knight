...
public class EasyEvent
{
    ...
    public EasyEvent(string eventPath)
    {
        path = eventPath;
        Setup();
    }
    ...
    public EasyEvent(string eventPath, UnityEngine.Object eventListener, bool listenForMarkers = true)
    {
    ...
    /// <param name="listenForMarkers">Whether a method should be invoked when a matching FMOD marker is passed.</param>
    public EasyEvent(string eventPath, UnityEngine.Object[] eventListeners, bool listenForMarkers = true)
    {
    ...
    public void Setup()
    {
        ...
        beatCallback = new FMOD.Studio.EVENT_CALLBACK(BeatEventCallback2);
        ...
        var tempString = path.Split('/');
        eventName = tempString[tempString.Length - 1];

        EasyEventUpdater.OnUpdate += Update;
    }

    public FMOD.RESULT BeatEventCallback2(FMOD.Studio.EVENT_CALLBACK_TYPE type, IntPtr instance, IntPtr parameterPtr)
    {
        FMOD.Studio.EventInstance ins;
        ins.handle = instance;
        return BeatEventCallback(type, ins, parameterPtr);
    }

    [AOT.MonoPInvokeCallback(typeof(FMOD.Studio.EVENT_CALLBACK))]
    public FMOD.RESULT BeatEventCallback(FMOD.Studio.EVENT_CALLBACK_TYPE type, FMOD.Studio.EventInstance instance, IntPtr parameterPtr)
    {
